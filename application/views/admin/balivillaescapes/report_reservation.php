...
<div class="table-responsive"> 
  	<table class="table table-bordered table-bve" id="reportReservation">
	  	<caption class="table-caption">The rates are quoted in AUD</caption>
	    <thead>
	      <tr>
	      	<th>No</th>
	        <th>Reservation</th>
				...
				<tr>
					...
					<td><?php echo $row->name;?></td>
						<?php
							...
							WHERE user.name='$row->name' AND balivillaescapes.enquiry_date!='0000-00-00' AND balivillaescapes.enquiry_date>='$enquiry_start' AND balivillaescapes.enquiry_date<='$enquiry_end' AND (status_enquiry.status_enquiry!='Proposed' OR status_enquiry.status_enquiry!='Extend Stay') AND balivillaescapes.balivillaescapes_status='1'");
						...
						?>
						<?php
							$reservation_booking = $this->db->query("SELECT * FROM balivillaescapes 
							LEFT JOIN user ON user.user_id=balivillaescapes.reservation_id 
							LEFT JOIN status_enquiry ON status_enquiry.status_enquiry_id=balivillaescapes.status_enquiry_id 
							WHERE user.name='$row->name' AND balivillaescapes.confirm_date>='$enquiry_start' AND balivillaescapes.confirm_date<='$enquiry_end' AND (status_enquiry.status_enquiry='Proposed' OR status_enquiry.status_enquiry='Confirm') AND balivillaescapes.balivillaescapes_status='1'");
						...
						?>
						<?php
							$reservation_revenue = $this->db->query("SELECT SUM(revenue_nett_aud) 
							AS revenue_reservation FROM balivillaescapes 
							LEFT JOIN user ON user.user_id=balivillaescapes.reservation_id 
							LEFT JOIN status_enquiry ON status_enquiry.status_enquiry_id=balivillaescapes.status_enquiry_id 
							...
							if ($result['0']->revenue_reservation!=0) {
								echo "<td>$ ".number_format($result['0']->revenue_reservation)."</b></td>";
							} else {
    							echo "<td>-</td>";
							}
						?>
	...
	<tfoot>
		<tr>
			...
			<?php
				$total_reservation_enquiry = $this->db->query("SELECT * FROM balivillaescapes 
				LEFT JOIN user ON user.user_id=balivillaescapes.reservation_id 
				LEFT JOIN status_enquiry ON status_enquiry.status_enquiry_id=balivillaescapes.status_enquiry_id 
				WHERE balivillaescapes.enquiry_date!='0000-00-00' AND balivillaescapes.enquiry_date>='$enquiry_start' AND balivillaescapes.enquiry_date<='$enquiry_end' AND balivillaescapes.balivillaescapes_status='1'");
			...
			?>
			<?php
				$total_reservation_booking = $this->db->query("SELECT * FROM balivillaescapes 
				LEFT JOIN user ON user.user_id=balivillaescapes.reservation_id 
				LEFT JOIN status_enquiry ON status_enquiry.status_enquiry_id=balivillaescapes.status_enquiry_id 
				WHERE balivillaescapes.confirm_date>='$enquiry_start' AND balivillaescapes.confirm_date<='$enquiry_end' AND (status_enquiry.status_enquiry='Proposed' OR status_enquiry.status_enquiry='Confirm') AND balivillaescapes.balivillaescapes_status='1'");
				
				if ($total_reservation_booking->num_rows()!=0) {
					echo "<th style='color: red;'><b>".$total_reservation_booking->num_rows()."</b></th>";
				} else {
					echo "<th>".$total_reservation_booking->num_rows()."</th>";
				}
			?>
			<?php
				$total_reservation_revenue = $this->db->query("SELECT SUM(revenue_nett_aud) AS total_reservation_revenue FROM balivillaescapes 
				LEFT JOIN user ON user.user_id=balivillaescapes.reservation_id 
				LEFT JOIN status_enquiry ON status_enquiry.status_enquiry_id=balivillaescapes.status_enquiry_id 
				WHERE balivillaescapes.confirm_date>='$enquiry_start' AND balivillaescapes.confirm_date<='$enquiry_end' AND (status_enquiry.status_enquiry='Proposed' OR status_enquiry.status_enquiry='Confirm' OR status_enquiry.status_enquiry='Extend Stay') AND balivillaescapes.balivillaescapes_status='1'");
