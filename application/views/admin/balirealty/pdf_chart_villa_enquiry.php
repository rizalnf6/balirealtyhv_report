...
<table class="header">
  <tr>
    <td class="header" width="50%"><h2>No. of Enquiry Villa <?php echo $villa_formated; ?></h2></td>
...
<table>
	<tr>
		<th align="center" bgcolor="#2a7fc2"><font color="#fff">Enquiry</font></th>
            <?php
              $i = 1;
...
?>
	...
	<tr>
		...
		<?php
            ...
            $max = 0;
            while($i <= 12) {
              	$table_villa_enquiry = $this->db->query("SELECT * FROM balirealtyhv 
                ...
                AND balirealtyhv.balirealtyhv_status='1'");
              	...
              	if ($table_villa_enquiry->num_rows() > $max) {
                	$max = $table_villa_enquiry->num_rows();
              	}
            $i++;
            }
            ...
            $i = 1;
            while($i <= 12) {
              	$max_table_villa_enquiry = $this->db->query("SELECT * FROM balirealtyhv 
                LEFT JOIN villa ON villa.villa_id=balirealtyhv.villa_id 
                LEFT JOIN status_enquiry ON status_enquiry.status_enquiry_id=balirealtyhv.status_enquiry_id 
                WHERE MONTH(balirealtyhv.enquiry_date)=$i 
                AND YEAR(balirealtyhv.enquiry_date)='$year'
                ...
                if ($max_table_villa_enquiry->num_rows() == $max) {
                echo '<td align="center" style="background-color: #e84c3d; color: #fff; font-weight: 700;">' . $max_table_villa_enquiry->num_rows(). '</td>'; 
                } else {
                  echo '<td align="center">' . $max_table_villa_enquiry->num_rows(). '</td>'; 
                }
            $i++;
            }       
          ?>
          ...
          <?php
            $total_daily_enquiry = $this->db->query("SELECT * FROM balirealtyhv 
                LEFT JOIN villa ON villa.villa_id=balirealtyhv.villa_id 
                LEFT JOIN status_enquiry ON status_enquiry.status_enquiry_id=balirealtyhv.status_enquiry_id 
                ...
                AND (status_enquiry.status_enquiry!='Proposed' 
					      ...
					      OR status_enquiry.status_enquiry!='Extend Stay')  
                AND balirealtyhv.balirealtyhv_status='1'");
              echo '<td align="center"><b>' .$total_daily_enquiry->num_rows(). '</b></td>';          
          ?>
	</tr>
<?php } ?>
</table>
