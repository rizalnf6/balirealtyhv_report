...
<div class="container-fluid">
	...
	<?php 
		...
		$delete_success = $this->session->flashdata('delete_success');
			...
			elseif (isset($add_success)) {
				echo "<div class='alert alert-success alert-dismissable'> <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;
				      </a><strong>Well done! </strong>".$this->session->flashdata('add_success')."</div>";
			} 
	?>
					...
					<div class="btn-group">
						...
						<span class="caret"></span>
							...
							<ul class="dropdown-menu dropdown-export">
								...
								<li>
									<a data-toggle="modal" data-target="#csv">
										<i class="fa fa-file" aria-hidden="true"></i></i> CSV
									</a>
								</li>
							</ul>
					</div>
							...
							<tr>
								...
								<td class="uppercase-name">
									<script>
										...
										$('[data-toggle="tooltipEnquiry"]').tooltip();   
										});
									</script>
									<a href="#" data-toggle="tooltipEnquiry" data-placement="right" 
									title="<?php echo Date("j M Y", strtotime($row->enquiry_date)) ?>">
										<span class="glyphicon glyphicon-time">
									</a>
									<?php 
										if ($row->balirealtyhv_status != "1") {
								...
								<?php 
									if ($row->checkin == "0000-00-00") {
										echo "<td style=color:#d9534f><b><i>T B A</i></b></td>";
									} else {
										echo "<td>".Date("j M Y", strtotime($row->checkin))."</td>";
									}
								?>
								<?php 
									if ($row->checkout == "0000-00-00") {
										echo "<td style=color:#d9534f><b><i>T B A</i></b></td>";
									} else {
										echo "<td>".Date("j M Y", strtotime($row->checkout))."</td>";
									}
								?>
								<?php if ($row->status_enquiry == "Extend Stay"): ?> 
									<td style=color:#d9534f><b><i>
										<script>
											$(document).ready(function(){
											$('[data-toggle="tooltipConfirm"]').tooltip();   
											});
										</script>
										<a href="#" data-toggle="tooltipConfirm" data-placement="right" 
										title="<?php echo Date("j M Y", strtotime($row->confirm_date)) ?>">
											<span class="glyphicon glyphicon-time">
										</a>
										<?php echo $row->status_enquiry; ?>
									</i></b></td>
								<?php endif ?>
								<?php if ($row->status_enquiry == "Proposed"): ?> 
									<td style=color:#5cb85c><b><i>
										<script>
											$(document).ready(function(){
											$('[data-toggle="tooltipConfirm"]').tooltip();   
											});
								...
								<?php endif ?>
								<?php if ($row->status_enquiry == "Confirm"): ?> 
									<td>
										<script>
											$(document).ready(function(){
											$('[data-toggle="tooltipConfirm"]').tooltip();   
								...
								<td class="no-border-right">
									<div class="action-button">
									    ...
									    <?php if ($this->session->userdata('level')=="admin"): ?>
											...
											<a href="<?php echo $button_print."/".$row->report_id."/".$row->villa."/".$row->guest_name; ?>">
			...
			<div class="modal-body" align="left">
			    <form method="get" class="form-horizontal" action="<?php echo $filter; ?>">
				    ...
				    <div class="form-group form-group-sm">
				        ...
				        <div class="col-xs-4">
				            <input type="date" class="form-control" name="confirm_start">
				        </div>
				        <div class="col-xs-4">
				            <input type="date" class="form-control" name="confirm_end">
				        </div>
				    </div>
				    <hr />
				    ...
				    <div class="form-group form-group-sm">
				        ...
				        <div class="col-xs-8">
				            <select class='form-control' name='reservation_name'>
				            <option value="">-</option>
									<?php
				            ...
				            <select class='form-control' name='villa'>
								<option value="">-</option>
									...
									foreach($list_villa as $row){
										echo 
										'<option value="' . $row[villa] . '" ' . ( isset( $default["villa"] ) && $default["villa"] == $row[villa] ? 'selected="selected"' : '' ) . '>' . $row[villa] . '</option>';	
									}
									?>
							</select>
				        </div>
				    ...
				    <div class="form-group form-group-sm">
				        ...
				        <div class="col-xs-8">
				            <select class='form-control' name='status_enquiry'>
				            	<option value="">-</option>
									<?php
									foreach($list_status_filter_confirm as $row){
										echo 
