...
var data = {
  ...
  datasets: [{
      ...
      data: [<?php
          ...
          while($i <= 31)
          {
              $chart_daily_enquiry2022 = $this->db->query("SELECT * FROM balirealtyhv 
                LEFT JOIN user ON user.user_id=balirealtyhv.reservation_id 
                LEFT JOIN status_enquiry ON status_enquiry.status_enquiry_id=balirealtyhv.status_enquiry_id 
                WHERE DAY(balirealtyhv.enquiry_date)=$i 
                AND MONTH(balirealtyhv.enquiry_date)=$month 
                ...
                AND (status_enquiry.status_enquiry!='Proposed' OR status_enquiry.status_enquiry!='Extend Stay') 
                AND balirealtyhv.balirealtyhv_status='1'");
                  echo '"' . $chart_daily_enquiry2022->num_rows(). '",';       
                  $i++;
          }       
                  ?>],
      spanGaps: true,
...
<div class="table-responsive">          
  <table class="table table-bordered table-brhv">
    <caption class="table-caption"><?php echo "Enquiry of ".date("F", mktime(0, 0, 0, $month, 10)); ?></caption>
      <thead>
        <tr>
          <th>Enquiry</th>
          <?php
      ...
      <tbody>
        ...
        <tr>
          ...
          <?php
            ...
            $max = 0;
            while($i <= 31) {
              $table_daily_enquiry2022 = $this->db->query("SELECT * FROM balirealtyhv 
              LEFT JOIN user ON user.user_id=balirealtyhv.reservation_id 
              LEFT JOIN status_enquiry ON status_enquiry.status_enquiry_id=balirealtyhv.status_enquiry_id 
              WHERE DAY(balirealtyhv.enquiry_date)=$i 
              AND MONTH(balirealtyhv.enquiry_date)=$month 
              ...
              if ($table_daily_enquiry2022->num_rows() > $max) {
                $max = $table_daily_enquiry2022->num_rows();
              }
            $i++;
            }
            ...
            $i = 1;
            while($i <= 31) {
              $max_table_daily_enquiry2022 = $this->db->query("SELECT * FROM balirealtyhv 
              LEFT JOIN user ON user.user_id=balirealtyhv.reservation_id 
              LEFT JOIN status_enquiry ON status_enquiry.status_enquiry_id=balirealtyhv.status_enquiry_id 
              WHERE DAY(balirealtyhv.enquiry_date)=$i 
              AND MONTH(balirealtyhv.enquiry_date)=$month 
              ...
              if ($max_table_daily_enquiry2022->num_rows() == $max) {
                echo '<td style="background-color: rgba(169, 68, 66, 0.5); color: #fff;">' . $max_table_daily_enquiry2022->num_rows(). '</td>'; 
              } else {
                echo '<td>' . $max_table_daily_enquiry2022->num_rows(). '</td>'; 
              }
            $i++;
            }       
          ?>
