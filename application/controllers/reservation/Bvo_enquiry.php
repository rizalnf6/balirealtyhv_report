...
class Bvo_enquiry extends CI_Controller {
	...
	public function new_enquiry($report_id=NULL) {
		...
		$config['base_url'] = base_url().'reservation/bvo_enquiry/new_enquiry';
		...
		$data['button_proccess'] = base_url().'reservation/bvo_enquiry/process_enquiry';

		$data['list_enquiry'] = $this->model_select_bvo->reservation_new_enquiry($config['per_page'], $report_id);
	...
	public function process_enquiry($report_id) {
		...
		redirect("reservation/bvo_enquiry/new_enquiry");
	}
	...
	public function enquiry($report_id=NULL) {
		...
		//pengaturan pagination
		$config['base_url'] = base_url().'reservation/bvo_enquiry/enquiry';
		$config['total_rows'] = $jml->num_rows();
		$config['per_page'] = '150';
		$config['first_page'] = 'Awal';
		$config['last_page'] = 'Akhir';
		...
		$data['list_enquiry'] = $this->model_select_bvo->reservation_enquiry($config['per_page'], $report_id);
	...
	public function details($report_id) {
		...
		$data['button_delete'] = base_url().'reservation/bvo_enquiry/delete';
		$data['button_print'] = base_url().'reservation/bvo_enquiry/print';
	...
	public function edit($report_id) {
		...
		if(isset($_POST['submit'])){
			$edit = $this->model_edit_bvo->reservation_edit_enquiry($_POST, $report_id);
			if ($edit) {
				$this->session->set_flashdata('edit_success', 'Data has been updated successfully');	
			}
		redirect("reservation/bvo_enquiry/enquiry");
		}
	...
	public function add() {
		...
		if(isset($_POST['submit'])){
			...
			$add_enquiry = $this->model_insert_bvo->add_enquiry($_POST);
			if ($add_enquiry) {
		...
		$data['header'] = 'reservation/header';
		$data['main_content'] = 'admin/balivillasonline/form_add_enquiry';
		$data['footer'] = 'footer';
