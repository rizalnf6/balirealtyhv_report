...
class Bve_enquiry extends CI_Controller {
	...
	public function new_enquiry($report_id=NULL) {
		...
		$config['base_url'] = base_url().'admin/bve_enquiry/new_enquiry';
		...
		$data['button_proccess'] = base_url().'admin/bve_enquiry/process_enquiry';

		$data['list_enquiry'] = $this->model_select_bve->admin_new_enquiry($config['per_page'], $report_id);
	...
	public function process_enquiry($report_id) {
		...
		redirect("admin/bve_enquiry/new_enquiry");
	}
	...
	public function enquiry($report_id=NULL) {
		...
		//pengaturan pagination
		$config['base_url'] = base_url().'admin/bve_enquiry/enquiry';
		$config['total_rows'] = $jml->num_rows();
		$config['per_page'] = '100';
		$config['first_page'] = 'Awal';
		$config['last_page'] = 'Akhir';
		...
		$data['list_enquiry'] = $this->model_select_bve->admin_enquiry($config['per_page'], $report_id);
	...
	public function details($report_id) {
		...
		$data['button_edit'] = base_url().'admin/bve_enquiry/edit';
		$data['button_propose'] = base_url().'admin/bve_propose_extend/propose';
		$data['button_delete'] = base_url().'admin/bve_enquiry/delete';
		$data['button_print'] = base_url().'admin/bve_enquiry/print';
	...
	public function edit($report_id) {
		...
		if(isset($_POST['submit'])){
			$edit = $this->model_edit_bve->admin_edit_enquiry($_POST, $report_id);
			if ($edit) {
				$this->session->set_flashdata('edit_success', 'Data has been updated successfully');	
			}
		redirect("admin/bve_enquiry/enquiry");
		}
