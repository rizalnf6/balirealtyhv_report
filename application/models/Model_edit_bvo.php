...
class Model_edit_bvo extends CI_Model {

	public function process_enquiry($report_id){
		$session_login = $this->session->userdata('name');

		$sql = $this->db->query("UPDATE balivillasonline SET status_enquiry_id='2' WHERE report_id= ".intval($report_id));
		return true;
	}
	...
	public function admin_edit_enquiry($post, $report_id){
		...
		$sql = $this->db->query("UPDATE balivillasonline SET 
				...
				remarks = $remarks,
				balivillasonline_last_modified = NOW(),
				balivillasonline_modified_by = '$modified_by'
